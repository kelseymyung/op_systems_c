...
static int print_path_info(struct fileinfo finfo); /* Prints formatted file information */ /* finfo.path is a pointer to the path, finfo.st is the stat struct */
...
static int read_file_list(DIR *dirp, struct fileinfo **file_list, size_t *file_count);    /* these functions are defined later in the code */
static void free_file_list(struct fileinfo **file_list, size_t file_count);
static int filecmp(void const *lhs, void const *rhs);
/* pointer to lhs, pointer to rhs*/

/* Some file-scoped objects avoid having to pass things between functions */
static int depth;
static struct tree_options opts;
static int cur_dir = AT_FDCWD;   /* current working directory of the caller: AT_FDCWD; cur_dir is probably an easier way to reference */
...
tree_print(char const *path, struct tree_options _opts)
{
  ...
  if ((finfo.path = strdup(path)) == NULL) goto exit; /* if path is NULL, go to exit */
  if (fstatat(cur_dir, path, &(finfo.st), AT_SYMLINK_NOFOLLOW) == -1) goto exit; /* if stat struct is error, go to exit */
  if (tree_print_recurse(finfo) == -1) goto exit; /* if recursive func returns error, go to exit */
exit:
  free(finfo.path);  
  return errno ? -1 : 0;
}
...
tree_print_recurse(struct fileinfo finfo)
{
  ...
  size_t file_count = 0;
  mode_t mode_type = finfo.st.st_mode;

  errno = 0;   

  /* TODO: implement dirsonly functionality here */
  if (opts.dirsonly && (S_ISDIR(mode_type) == 0)) goto exit;  /* S_ISDIR returns non-zero if directory, returns 0 if not a directory */
  
  /* TODO: print indentation */
  for (int i = 0; i < opts.indent * depth; ++i) putchar(' ');   

  /* TODO: print the path info */
  print_path_info(finfo); /* Prints formatted file information */ 
  if (S_ISDIR(mode_type) == 0) putchar('\n');
  

  /* TODO: continue ONLY if path is a directory */
  if (S_ISDIR(mode_type) == 0) goto exit;
...
exit: 
  ...
  if (dirp != NULL) {       /* if dirp is pointing to directory */
    closedir(dirp);
    errno = 0;              /* reset errno */
    }
  cur_dir = sav_dir;        /* recursive-out of function */
  return errno ? -1 : 0;
}
...
{
  char sep = '[';
  if (opts.perms) {             /* mode_string returns string with permissions info */
  ...
  if (opts.user) {
    ...
    uid_t userid = finfo.st.st_uid;
    
    userinfo = getpwuid(userid);

    if (printf("%c%s", sep, userinfo->pw_name) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.group) {
    /*  Hint: getgrgid(3) */
    struct group *groupinfo;
    gid_t groupid = finfo.st.st_gid;
    
    groupinfo = getgrgid(groupid);
    if (printf("%c%s", sep, groupinfo->gr_name) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.size) {
    /*  Hint: stat.h(0p) */
...
read_file_list(DIR *dirp, struct fileinfo **file_list, size_t *file_count)
{
  for (;;) {
    ...
    if (de->d_name[0] == '.' && !opts.all) continue;        /* if first character is . and opts.all is NOT selected */
